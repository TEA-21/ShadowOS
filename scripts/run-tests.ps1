...
Write-Host "`n[1/14] Running shadow-core unit & protocol integration tests..." -ForegroundColor Yellow
cargo test -p shadow-core --test protocol_test -- --nocapture

Write-Host "`n[2/14] Running shadow-core NFR framing benchmarks..." -ForegroundColor Yellow
cargo test -p shadow-core --test benchmark_nfr -- --nocapture

Write-Host "`n[3/14] Running shadow-vmm lifecycle state tests..." -ForegroundColor Yellow
cargo test -p shadow-vmm --test vmm_lifecycle_tests -- --nocapture

Write-Host "`n[4/14] Running shadow-vsock channel queue tests..." -ForegroundColor Yellow
cargo test -p shadow-vsock --test vsock_tests -- --nocapture

Write-Host "`n[5/14] Running shadow-vsock streaming bash execution tests..." -ForegroundColor Yellow
cargo test -p shadow-vsock --test streaming_exec_tests -- --nocapture

Write-Host "`n[6/14] Running shadow-vmm virtio-fs & DAX I/O tests..." -ForegroundColor Yellow
cargo test -p shadow-vmm --test virtiofs_tests -- --nocapture

Write-Host "`n[7/14] Running shadow-vmm cold boot provisioning benchmarks..." -ForegroundColor Yellow
cargo test -p shadow-vmm --test cold_boot_benchmarks -- --nocapture

Write-Host "`n[8/14] Running shadow-cow ephemeral isolation & zero pollution tests..." -ForegroundColor Yellow
cargo test -p shadow-cow --test cow_isolation_tests -- --nocapture

Write-Host "`n[9/14] Running shadow-snapshot state rollback & checkpoint tests..." -ForegroundColor Yellow
cargo test -p shadow-snapshot --test snapshot_rollback_tests -- --nocapture

Write-Host "`n[10/14] Running shadow-cli harness & auto-approve execution tests..." -ForegroundColor Yellow
cargo test -p shadow-cli --test cli_execution_tests -- --nocapture

Write-Host "`n[11/14] Running shadow-tui diff inspector & keyboard simulation tests..." -ForegroundColor Yellow
cargo test -p shadow-tui --test tui_interaction_tests -- --nocapture

Write-Host "`n[12/14] Running shadow-mcp Model Context Protocol (MCP) server tests..." -ForegroundColor Yellow
cargo test -p shadow-mcp --test mcp_protocol_tests -- --nocapture

Write-Host "`n[13/14] Running shadow-vmm headless virtual display & raw CDP tests..." -ForegroundColor Yellow
cargo test -p shadow-vmm --test display_cdp_tests -- --nocapture

Write-Host "`n[14/14] Running shadow-mcp visual browser tools tests..." -ForegroundColor Yellow
